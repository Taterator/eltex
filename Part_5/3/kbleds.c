...
static struct kobj_attribute foo_attribute = __ATTR(test, 0664, foo_show,
                                                    foo_store);
...
static void my_timer_func(struct timer_list *ptr)
{
  ...
  (my_driver->ops->ioctl)(vc_cons[fg_console].d->port.tty, KDSETLED,
                          *pstatus);
  printk(KERN_INFO "led status: %x\n", *pstatus); // Добавил чтобы отслеживать светодиоды,
                                                  //  т.к. у меня на ноутбуке только капс
...
static void __exit sys_led_cleanup(void)
{
  sys_exit();
  kbleds_cleanup();
}
module_init(sys_led_init);
module_exit(sys_led_cleanup);
